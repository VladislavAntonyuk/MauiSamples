...
public partial class MainPageViewModel : ObservableObject {
  ...
  [RelayCommand]
  void Quit() { Application.Current?.Quit(); }
  ...
  void ToggleTheme() {
    if (Application.Current is not null) {
      Application.Current.UserAppTheme =
          Application.Current.UserAppTheme == AppTheme.Light ? AppTheme.Dark
                                                             : AppTheme.Light;
    }
  }
  ...
  [RelayCommand]
  void SetEraser() {
    LineColor = Background is SolidColorBrush solidColorBrush
  ...
  void Rotate(double angle) {
    ...
    foreach (var line in oldLines) {
      ...
      foreach (var point in points) {
        line.Points.Add(RotatePoint(
            point, new PointF(canvasWidth / 2, canvasHeight / 2), angle));
      }
    }

    Lines = oldLines.ToObservableCollection();

    static PointF RotatePoint(PointF pointToRotate, PointF centerPoint,
  ...
  async Task PasteFromClipboard(CancellationToken cancellationToken) {
    ...
    if (image is null) {
      await Toast.Make("Clipboard doesn't contain images")
  ...
  async Task Open(CancellationToken cancellationToken) {
    await using var stream =
        await dialogService.OpenFileDialog(cancellationToken);
    if (stream == Stream.Null) {
      return;
    }

    try {
      var projectState = await serializerService.Deserialize<ProjectState>(
  ...
  async Task Save(CancellationToken cancellationToken) {
    var projectState =
        new ProjectState { Figures = _figures.ToArray(),
                           Lines = Lines.ToArray(), LineColor = LineColor,
                           LineWidth = LineWidth, Background = Background };
    await using var stream =
        await serializerService.Serialize(projectState, cancellationToken);
  ...
  async Task SaveToFile(Stream stream, string fileExtension,
                        CancellationToken cancellationToken) {
    var isSaved = await dialogService.SaveFileDialog(stream, fileExtension,
                                                     cancellationToken);
    if (isSaved) {
      await Toast.Make("File is saved", ToastDuration.Long)
          .Show(cancellationToken);
    }
  }
}